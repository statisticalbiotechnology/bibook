...
**Semi-global alignment** allows unpenalised overhangs at the ends of both sequences, so that only the overlapping regions are aligned. Use it when the two sequences are expected to overlap — two sequencing reads from the same locus, say — rather than to correspond over their full lengths. Note that the match has to reach a boundary of each sequence: a region matching in the *middle* of a long sequence is a job for local alignment. See [variants of Needleman–Wunsch](../pairwise/semi).
...
* - **Semi-global**
  ...
  - Read overlaps in assembly, sequences running past each other's ends
